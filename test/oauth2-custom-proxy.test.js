...
test.describe('OAuth 2.0 Proxy', () => {
  test.describe('custom grant', () => {
    ...
    const baseConfig = /** @type IOAuth2Authorization */ ({
      ...
      scopes: ['a', 'b'],
      accessTokenUri: 'http://localhost:8000/oauth2/custom-grant',
    });
    ...
    test('handles invalid grant', async () => {
      const config = {
        ...baseConfig,
        grantType: 'other-custom',
      };
      const tokenInfo = await proxy.proxyOauth2(config);
      expect(tokenInfo.code).toEqual('invalid_grant');
      expect(tokenInfo.error).toEqual(true);
    });
